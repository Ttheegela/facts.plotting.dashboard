...
_FALLBACK_COLORS = ["teal", "olive", "coral", "slategray", "darkviolet", "steelblue"]
...
def _store_result(data_dict, location_meta, result, key_prefix):
    ...
    for li, loc_id in enumerate(result["locations"]):
        ...
        data_dict[key] = {
            "years": result["years"],
            "med":   q[2, :, li].tolist(),   # p50 — median projection
            "lo":    q[1, :, li].tolist(),   # p17 — lower likely range
            "hi":    q[3, :, li].tolist(),   # p83 — upper likely range
            "vlo":   q[0, :, li].tolist(),   # p05 — very likely lower bound
            "vhi":   q[4, :, li].tolist(),   # p95 — very likely upper bound
        }
...
def _build_stacked_bar_section(
    ...
    Args:
        data_dict:        Full data dict keyed by {ssp}|{component}|{wf}|{scale}|{loc_id}.
        years:            Full year list.
        ssps:             List of SSP tag strings (e.g. ['ssp126', 'ssp245', ...]).
        wfs:              List of workflow ID strings.
        loc_options_all:  (value, label) pairs for all locations incl. global (loc_id=-1).
        location_meta_js: JS-side location metadata dict keyed by str(loc_id).
    ...
    SSP_STACK_COLORS = {
        "ssp119": "#1f77b4",
        "ssp126": "#17becf",   # teal/blue
        "ssp245": "#9467bd",   # purple
        "ssp370": "#ff7f0e",   # orange
        "ssp585": "#8b1a1a",   # dark red — always on top
        "ssp534": "#bcbd22",
    }
    ...
    # -- Helper: p50 median (mm) --
    def _med(ssp, comp, wf, scale, loc_id, year):
        lookup = -1 if scale == "global" else loc_id
        d = data_dict.get(f"{ssp}|{comp}|{wf}|{scale}|{lookup}")
        if not d:
            return 0.0
        try:
            return d["med"][d["years"].index(year)]
        except ValueError:
            return 0.0
    ...
    init_data = {
        "x":         list(range(n)),
        "loc_names": loc_names,
        "loc_ids":   [str(lid) for lid in loc_ids],
        "loc_lat":   loc_lat,
        "loc_lon":   loc_lon,
    }
    for ssp in stack_ssps:
        vals = [_med(ssp, default_comp, default_wf, default_scale, lid, default_year)
                for lid in loc_ids]
        init_data[ssp]               = vals           # cumulative stack column
        init_data[f"val_{ssp}"]      = vals           # individual value column (for hover)
        init_data[f"val_{ssp}_fmt"]  = [f"{v:.1f}" for v in vals]  # formatted string
    ...
    p_bar = figure(
        ...
        height=520,
        title=(
            f"Relative Sea Level Projections at {default_year} — "
            f"Stacked SSPs per Indian Ocean Tide Gauge"
            f"  ({default_wf}, {default_comp}, {default_scale})"
        ),
    ...
    for ssp in stack_ssps:
        ...
        tooltip_rows.append((label, f"@val_{ssp}_fmt mm"))
    ...
    renderers = p_bar.vbar_stack(
        ...
        x="x",
        width=0.7,
        color=[SSP_STACK_COLORS.get(s, "#aaaaaa") for s in stack_ssps],
        source=source_bar,
        legend_label=[SSP_LABELS.get(s, s) for s in stack_ssps],
    )

    # x-axis: rotated location name labels
    p_bar.xaxis.ticker = list(range(n))
    p_bar.xaxis.major_label_overrides = {i: name for i, name in enumerate(loc_names)}
    p_bar.xaxis.major_label_orientation = 1.0   # ~57 degrees

    # Legend outside the plot area (right side) so it does not overlap bars
    p_bar.add_layout(p_bar.legend[0], "right")
    p_bar.legend[0].title           = "SSP Scenario"
    p_bar.legend[0].title_text_font_style = "bold"
    p_bar.legend[0].label_text_font_size  = "12px"
    ...
    const new_data = {
        x:         source_bar.data.x.slice(),
        loc_names: source_bar.data.loc_names.slice(),
        loc_ids:   source_bar.data.loc_ids.slice(),
        loc_lat:   source_bar.data.loc_lat.slice(),
        loc_lon:   source_bar.data.loc_lon.slice(),
    };
    for (const ssp of stack_ssps) {
        new_data[ssp]              = [];
        new_data["val_" + ssp]     = [];
        new_data["val_" + ssp + "_fmt"] = [];
    }

    for (let i = 0; i < loc_ids_arr.length; i++) {
        const loc_id = (scale === "global") ? -1 : loc_ids_arr[i];
        for (const ssp of stack_ssps) {
            ...
            if (d) {
                ...
                if (idx !== -1) val = d.med[idx];
            }
            new_data[ssp].push(val);
            new_data["val_" + ssp].push(val);
            new_data["val_" + ssp + "_fmt"].push(val.toFixed(1));
        }
    }
    ...
    p_bar.title.text = "Relative Sea Level Projections at " + year +
                       " — Stacked SSPs per Indian Ocean Tide Gauge" +
                       "  (" + wf + ", " + comp + ", " + scale + ")";
    """
    ...
    bar_head = Div(text="""
<div style="margin-top:32px;margin-bottom:6px;">
  <u>FACTS <b>sea-level projections — stacked SSPs</b></u> (STACKED BAR CHART)<br>
  Each bar = one tide gauge location. Stacks = SSP scenarios,
  <b>SSP1-2.6 at bottom → SSP5-8.5 on top</b>.<br>
  Select <b>workflow</b>, <b>component</b>, <b>scale</b>, and <b>year</b>.
  Hover over any bar to see location coordinates and each SSP's individual RSL value (mm).
  Click legend items to show/hide individual scenarios.
</div>
""", width=1100)

    return column(bar_head, row(wf_sel, comp_sel, scale_sel, year_sel), p_bar)
